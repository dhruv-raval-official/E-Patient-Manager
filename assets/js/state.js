...
var getpincodes = function($city) {
    $url = 'https://api.postalpincode.in/postoffice/' + $city;
    console.log($url);
    $.getJSON($url, function(data) {
        var dataset = data[0]['PostOffice'];
        var options = '';
        var items = [];
        $.each(dataset, function(key, val) {
            items.push(val['Pincode']);
        });
        items = getUnique(items);
        options += '<option value="SELECT PINCODE">SELECT PINCODE</option>';
...
function set_city($val) {
    ...
    if ($val == 'Gujarat') {
        var gujarat = ["Ahmedabad", "Ambaji", "Amreli", "Anand", "Aravalli", "Banaskantha", "Bharuch", "Bhavnagar", "Dahod", "Dang", "Gandhinagar", "Jamnagar", "Junagadh",
            "Kutch", "Kheda", "Mahesana", "Narmada", "Navsari", "Patan", "Panchmahal", "Porbandar", "Rajkot", "Sabarkantha", "Surendranagar", "Surat", "Tapi", "Vadodara", "Valsad"
        ];
        $(function() {
            var options = '';
    ...
    if ($val == 'Karnataka') {
        var karnataka = ["Bagalkot", "Bangalore", "Bangalore Urban", "Belgaum", "Bellary", "Bidar", "Bijapur", "Chamarajnagar", "Chikkamagaluru", "Chikkaballapur",
            "Chitradurga", "Davanagere", "Dharwad", "Dakshina Kannada", "Gadag", "Gulbarga", "Hassan", "Haveri", "Kodagu",
